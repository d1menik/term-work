...
if (isset($_POST['playlistId']) && isset($_POST['songId'])) {
    ...
    $stmt = $conn->prepare("SELECT MAX(playlistOrder) as playlistOrder FROM listSongs WHERE playlist_id=:playlist_id");
    $stmt->execute(array(':playlist_id' => $playlist_id));
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    $order = $result['playlistOrder']  + 1;

    var_dump($order);
    $stmt1 = $conn->prepare("INSERT INTO listSongs (song_id, playlist_id, playlistOrder) VALUES (:song_id, :playlist_id, :playlistOrder)");
